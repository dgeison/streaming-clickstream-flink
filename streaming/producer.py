...
def publicar_clickstream(bootstrap_servers, duracao_segundos, seed=None, sleep_fn=time.sleep):
    ...
    producer = Producer({"bootstrap.servers": bootstrap_servers, "acks": "all", "linger.ms": 10})
    estados = {}
    inicio = time.time()
    eventos_enviados = 0
    ...
    while time.time() - inicio < duracao_segundos:
        ...
        producer.produce(TOPICO_CLICKSTREAM, key=user_id, value=json.dumps(evento, ensure_ascii=False))
        producer.poll(0)
        eventos_enviados += 1
